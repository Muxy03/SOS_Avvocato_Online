import { u as push, z as getContext, y as escape_html, w as pop, F as attr } from './index-CJFxRk-N.js';
...
function _page($$payload, $$props) {
  push();
  const tmp = getContext("App"), isOnline = tmp.isOnline, isLoading = tmp.isLoading, user = tmp.user;
  tmp.error;
  ...
  if (isLoading.value) {
    ...
    if (user.value !== null) {
      ...
      $$payload.out.push(`<!--]--> <a href="/home" target="_self" rel="noopener noreferrer" aria-label="/home" class="w-full"><div class="flex h-[60px] w-[280px] items-center gap-1 rounded-2xl bg-gray-100 dark:bg-gray-700"><div class="flex h-10 w-10 flex-1/4 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-sm font-bold text-white">${escape_html(getInitials(user.value?.displayName ?? "", user.value?.email ?? ""))}</div> <div class="w-full flex-3/4 text-center"><h2 class="text-xl font-bold break-words text-gray-800 dark:text-gray-100">${escape_html(user.value?.displayName)}</h2> <p class="text-sm break-all text-gray-500 dark:text-gray-400">${escape_html(user.value?.email)}</p></div></div></a></div></div> <div class="flex flex-col gap-4"><button type="button" class="rounded-xl border-2 border-gray-200 bg-gray-100 px-6 py-4 text-base font-semibold text-slate-600 transition hover:border-gray-300 hover:bg-gray-200">Disconnetti</button></div></div>`);
    } else {
      $$payload.out.push("<!--[!-->");
    }
    $$payload.out.push(`<!--]-->`);
...
//# sourceMappingURL=_page.svelte-CuZfkyuX.js.map
