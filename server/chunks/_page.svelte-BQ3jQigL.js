import { O as push, $ as await_block, Q as pop, Z as attr, V as escape_html } from './index-DwtfPoHx.js';
import { n as navigating } from './index3-D88g52U-.js';
  ...
  if (isLoading.value) {
    ...
    if (user.value !== void 0) {
      ...
      await_block(
        ...
        (_) => {
          $$payload.out.push(`<a href="/home" target="_self" rel="noopener noreferrer" aria-label="/home" class="w-full"><div class="flex h-[60px] w-[280px] items-center gap-1 rounded-2xl bg-blue-500"><div class="flex h-10 w-10 flex-1/4 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-sm font-bold text-white">${escape_html(getInitials(user.value.FullName, user.value.email))}</div> <div class="w-full flex-3/4 text-center"><h2 class="text-xl font-bold break-words text-white">${escape_html(user.value.FullName)}</h2> <p class="text-sm break-all text-white">${escape_html(user.value.email)}</p></div></div></a>`);
        }
      );
...
//# sourceMappingURL=_page.svelte-BQ3jQigL.js.map
